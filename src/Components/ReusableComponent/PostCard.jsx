...
export default function CardPost({
  ...
  const [editImagePreview, setEditImagePreview] = useState(posts?.image || null);
  const fileInputRef = useRef(null);
  const textInputRef = useRef(null);

  useEffect(() => { if (posts.bookmarked === true) setBookmarked(true); }, [posts.bookmarked]);
  useEffect(() => { if (posts.sharesCount > 0) setIsShared(true); }, [posts.sharesCount]);
  useEffect(() => { setLiked(posts.likes?.includes(loggedId) || false); }, [posts.likes, loggedId]);

  function deletePost() {
    return axios.delete(`https://route-posts.routemisr.com/posts/${posts._id}`, {
  ...
  function prepareUpdatePost() {
    ...
    if (textInputRef.current.value) formData.append("body", textInputRef.current.value);
    if (fileInputRef.current.files[0]) formData.append("image", fileInputRef.current.files[0]);
    return formData;
  }

  function updatePost() {
    return axios.put(`https://route-posts.routemisr.com/posts/${posts._id}`, prepareUpdatePost(), {
      headers: { Authorization: `Bearer ${localStorage.getItem("Token")}` },
    });
  }

  function likePost() {
    return axios.put(`https://route-posts.routemisr.com/posts/${posts._id}/like`, {}, {
      headers: { Authorization: `Bearer ${localStorage.getItem("Token")}` },
    });
  }

  function sharePost() {
    return axios.post(`https://route-posts.routemisr.com/posts/${posts._id}/share`,
  ...
  function bookMarkPost() {
    return axios.put(`https://route-posts.routemisr.com/posts/${posts._id}/bookmark`, {}, {
      headers: { Authorization: `Bearer ${localStorage.getItem("Token")}` },
    });
  }
  ...
  const { isPending: delPending, mutate: delMutate } = useMutation({
    ...
    onSuccess: () => {
      ...
      onClose();
      addToast({ title: "Post deleted", description: "Removed successfully", color: "success", timeout: 3000 });
      if (location.pathname.includes("/postdetails")) navigate("/explore");
    },
    onError: () => addToast({ title: "Failed to delete", description: "Something went wrong, try again", color: "danger", timeout: 3000 }),
  });
  ...
  const { isPending: editPending, mutate: editMutate } = useMutation({
    ...
    onSuccess: () => {
      ...
      query_A.invalidateQueries({ queryKey: ["getHomePosts"] });
      onEditClose();
      addToast({ title: "Post Updated", description: "Updated successfully", color: "success", timeout: 3000 });
    },
    onError: () => addToast({ title: "Failed to Update", description: "Something went wrong, try again", color: "danger", timeout: 3000 }),
  });
  ...
  const { mutate: shareMutate } = useMutation({
    ...
    onError: () => addToast({ title: "Failed to Share", description: "Something went wrong, try again", color: "danger", timeout: 3000 }),
  });
  ...
  return (
    <>
        ...
        <ModalContent>
          <ModalBody>
            <div className="flex items-center gap-2 mb-2">
              <Image src={user?.photo} width={32} height={32} radius="full" className="ring-2 ring-purple-200 shrink-0" />
            ...
            <div className="mt-3">
              <AnimatePresence mode="wait">
                {editImagePreview ? (
                  ...
                  >
                    <img src={editImagePreview} alt="preview" className="w-full max-h-48 sm:max-h-56 object-cover" />
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all duration-300 flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100">
                      <button onClick={() => fileInputRef.current?.click()} className="bg-white/90 text-purple-600 font-medium text-xs px-3 py-2 rounded-xl shadow-md hover:bg-white transition">Change</button>
                      <button onClick={() => { setEditImagePreview(null); if (fileInputRef.current) fileInputRef.current.value = ""; }} className="bg-white/90 text-red-400 font-medium text-xs px-3 py-2 rounded-xl shadow-md hover:bg-white transition">Remove</button>
                    </div>
                  </motion.div>
                ) : (
                  <motion.button
                  ...
                  >
                    ...
                    <p className="text-sm text-purple-400 font-medium">Add a photo</p>
                  </motion.button>
                )}
              </AnimatePresence>
              <input ref={fileInputRef} type="file" accept="image/*" className="hidden"
                onChange={(e) => setEditImagePreview(URL.createObjectURL(e.target.files[0]))} />
            </div>
          </ModalBody>

          <ModalFooter>
            <Button variant="flat" onPress={onEditClose} className="flex-1 rounded-xl bg-stone-100 text-stone-600 hover:bg-stone-200">Cancel</Button>
            <Button
              onPress={editMutate}
              isLoading={editPending}
              isDisabled={editPending}
              spinner={<svg className="animate-spin h-4 w-4 text-white" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" /></svg>}
        ...
        <ModalContent>
          <ModalHeader className="flex flex-col items-center gap-3 px-4 sm:px-5">
            ...
            <div className="text-center">
              <p className="text-base font-semibold text-stone-800">Delete Post?</p>
              <p className="text-sm text-stone-400 font-normal mt-1">This action can't be undone.</p>
            </div>
          </ModalHeader>
          <ModalBody />
          <ModalFooter>
            <Button variant="flat" onPress={onClose} isDisabled={delPending} className="flex-1 rounded-xl bg-stone-100 text-stone-600 font-medium hover:bg-stone-200">Cancel</Button>
            <Button color="danger" onPress={() => delMutate()} isLoading={delPending} className="flex-1 rounded-xl font-medium">
      ...
      <Card className={`w-full mx-auto max-w-185 ${margin} rounded-2xl shadow-sm border border-stone-100 bg-white hover:shadow-md transition-shadow duration-300`}>
        <CardHeader className="flex gap-2 sm:gap-3 px-3 sm:px-5 pt-4 sm:pt-5 pb-3">
          ...
          <div className="flex flex-col flex-1 min-w-0">
            <p className="font-semibold text-stone-800 text-sm truncate">{posts.user.name}</p>
            <p className="text-xs text-stone-400">{formatDate(posts.createdAt)}</p>
          </div>
          ...
          <Dropdown>
            ...
            <DropdownMenu aria-label="Post Actions" className="min-w-36">
              {loggedId === userId ? (
                <>
                  <DropdownItem key="edit" onPress={onEditOpen} startContent={<i className="fa-regular fa-pen-to-square text-blue-400 w-4" />} className="text-stone-600 hover:text-blue-500">Edit</DropdownItem>
                  <DropdownItem onPress={onOpen} key="delete" startContent={<i className="fa-regular fa-trash-can text-red-400 w-4" />} className="text-danger" color="danger">Delete</DropdownItem>
                </>
              ) : (
                <DropdownItem
                  key="bookmark"
                ...
                >
                  <span className="font-medium">{isBookmarked ? "Saved!" : "Bookmark"}</span>
                </DropdownItem>
              )}
            </DropdownMenu>
          </Dropdown>
        ...
        <CardBody className="px-3 sm:px-5 py-2 gap-3">
          {posts.body && (
            <p className="text-stone-700 text-sm leading-relaxed wrap-break-words">{posts.body}</p>
          )}
          {posts.image && (
        ...
        <CardFooter className="flex-col gap-3 px-2 sm:px-5 pb-3 sm:pb-4 pt-2">
          <div className="w-full flex justify-around gap-1">
            ...
            >
              {isLiked && ["🎉", "✨", "💙", "⭐", "🎊"].map((emoji, i) => (
                <motion.span key={i} className="absolute text-xs pointer-events-none" style={{ left: "50%", top: "50%" }}
                  initial={{ x: 0, y: 0, opacity: 1, scale: 0.5 }}
                  animate={{ x: (i % 2 === 0 ? 1 : -1) * (20 + i * 14), y: -(30 + i * 10), opacity: 0, scale: 1.3, rotate: i * 45 }}
                  transition={{ duration: 0.6, ease: "easeOut" }}>{emoji}</motion.span>
              ))}
              <motion.i
                key={isLiked ? "liked" : "unliked"}
                className={isLiked ? "fa-solid fa-thumbs-up" : "fa-regular fa-thumbs-up"}
                animate={isLiked ? { rotate: [0, -20, 15, -10, 8, 0], scale: [1, 1.4, 1.1, 1.3, 1] } : { rotate: 0, scale: 1 }}
                transition={{ duration: 0.5, ease: "easeInOut" }}
              />
              <motion.span key={isLiked ? "yes" : "no"} initial={{ y: 8, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.2 }}>
                {isLiked ? "Liked!" : "Like"}
              </motion.span>
            </motion.button>

            {/* Comment */}
            <motion.div whileTap={{ scale: 0.85, rotate: -5 }} whileHover={{ scale: 1.05 }} transition={{ type: "spring", stiffness: 400, damping: 10 }}>
              <Link
            ...
            {loggedId === userId ? (
              ...
              >
                ...
                <motion.i
                  ...
                  animate={isShared ? { rotate: [0, -20, 15, -10, 8, 0], scale: [1, 1.4, 1.1, 1.3, 1] } : { rotate: 0, scale: 1 }}
                  transition={{ duration: 0.5, ease: "easeInOut" }}
                />
                <motion.span key={isShared ? "yes" : "no"} initial={{ y: 8, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.2 }}>
          ...
          {showAllComments
            ? comments.map((comment) => (
                <Comment key={comment._id} postId={posts.id} comment={comment} />
              ))
        ...
        <CreateComment
          id={posts._id}
          userId={userId}
          queryKey={showAllComments ? [`getPostComment${posts.id}`] : ["getAllPosts"]}
        />
      </Card>
    </>
  );
